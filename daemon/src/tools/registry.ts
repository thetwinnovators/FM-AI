...
const TOOL_META: Record<string, { displayName: string; description: string; risk: RiskLevel; group: CapabilityGroup }> = {
  ...
  'file.write':          { displayName: 'Write file',        description: 'Write or append text to a file. Params: path, content, mode (overwrite|append)', risk: 'write', group: 'file' },
  'file.edit':           { displayName: 'Edit file',         description: 'Find-and-replace in a file. Params: path, old_string (exact unique text to replace), new_string (replacement). old_string must appear exactly once.', risk: 'write', group: 'file' },
  'file.delete':         { displayName: 'Delete file',       description: 'Delete a file or directory. Params: path, recursive (bool)', risk: 'publish', group: 'file' },
  'system.exec':         { displayName: 'Run allowlisted command', description: 'Run an allowlisted binary. Params: command (binary name e.g. "git"), args (string array e.g. ["status"]), cwd (optional path)', risk: 'write', group: 'system' },
...
export function buildRegistry(opts: RegistryOptions): ToolRegistry {
  ...
  const handlers: Record<string, ToolHandler> = {
    ...
    'file.edit':           async (p) => file.edit(p as any),
